...
export default function ItemDetails({ Navigation, route }) {
    ...
    const selectedItemCategory = route.params[1].filter((e) => e.id !== selectedItemId);
                ...
                />
                    : selectedItemCategory.length > 0
                        ? <ScrollView style={globalStyles.scrollView}>
                            <Text style={globalStyles.categoryHeading}>{selectedItem[0].category}</Text>
                            <View style={styles.selectedItemView}>
                                <View style={styles.selectedItemUpperView}>
                                    ...
                                    <View style={styles.selectedItemRightView}>
                                        <Text style={globalStyles.price}>${selectedItem[0].price}</Text>
                                        <Text style={globalStyles.text}>{selectedItem[0].title/* .length < 30 ? slice(0, 30) */}</Text>
                                    </View>
                                </View>
                                <View style={styles.selectedItemLowerView}>
                                    <Text style={globalStyles.text}>{selectedItem[0].description}</Text>
